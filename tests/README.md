...
cargo test -p marser --features "parser-erased parser-trace json-testsuite"
```

- **`parser-erased`** — convention for this repo; needed by the example grammars pulled into tests.
...
For **day-to-day** runs you usually enable only `parser-erased` (see the repo `README.md`).
...
```bash
cargo test -p marser --features parser-erased
```
...
```bash
cargo test -p marser --features parser-erased --test json_harness
```

Compile-fail UI tests (`trybuild`):

```bash
cargo test -p marser --features parser-erased --test capture_ui
```
...
| `trace_harness.rs` | Parser tracing (`parse_with_trace`, markers, file output). **Requires** `parser-trace` (and usually `parser-erased` for the rest of the suite you run in the same command). |
...
```bash
TRYBUILD=overwrite cargo test -p marser --features parser-erased --test capture_ui
```
...
2. Run the harness:
   ...
   ```bash
   cargo test -p marser --features "parser-erased json-testsuite" --test json_testsuite
   ```
...
```bash
cargo test -p marser --features "parser-erased parser-trace" --test trace_harness
```
